...
class NegociacaoFormView extends TPage
{
    ...
    public function __construct( $param )
    {
        ...
        $text1 = new TTextDisplay($negociacao->id, '', '12px', '');
        $label6 = new TLabel("Tipo da negociação:", '', '12px', '');
        $text5 = new TTextDisplay($negociacao->tipo_negociacao->nome, '', '12px', '');
        $label9 = new TLabel("Descrição:", '', '12px', '');
        $text9 = new TTextDisplay($negociacao->descricao, '', '12px', '');
        $label2 = new TLabel("Cliente:", '', '12px', '');
        $text2 = new TTextDisplay($negociacao->cliente->nome, '', '12px', '');
        $label22 = new TLabel("Vendedor:", '', '12px', '');
        $text3 = new TTextDisplay($negociacao->vendedor->nome, '', '12px', '');
        $label4 = new TLabel("Origem da negociação:", '', '12px', '');
        $text4 = new TTextDisplay($negociacao->origem_negociacao->nome, '', '12px', '');
        $label8 = new TLabel("Fase da negociação:", '', '12px', '');
        $text6 = new TTextDisplay($negociacao->estado_negociacao->nome, '', '12px', '');
        $label7 = new TLabel("Início da negociação:", '', '12px', '');
        $text7 = new TTextDisplay(TDateTime::convertToMask($negociacao->dt_inicio_negociacao, 'yyyy-mm-dd hh:ii', 'dd/mm/yyyy hh:ii'), '', '12px', '');
        $label11 = new TLabel("Fim da negociação:", '', '12px', '');
        $text8 = new TTextDisplay(TDateTime::convertToMask($negociacao->dt_fim_negociacao, 'yyyy-mm-dd hh:ii', 'dd/mm/yyyy hh:ii'), '', '12px', '');
        $label10 = new TLabel("Observação:", '', '12px', '');
        $text10 = new TTextDisplay($negociacao->obs, '', '12px', '');
        ...
        $column_produto_nome = $this->negociacao_produto_negociacao_id_list->addQuickColumn("Produto", 'produto->nome', 'left');
        ...
        $column_dt_contato_transformed->setTransformer(function($value, $object, $row, $cell = null, $last_row = null)
        {
            if(!empty(trim($value)))
            {
                try
