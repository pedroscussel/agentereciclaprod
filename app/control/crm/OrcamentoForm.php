...
class OrcamentoForm extends TPage
{
    ...
    public function __construct( $param )
    {
        ...
        $criteria_estado_orcamento_id = new TCriteria();
        $criteria_orcamento_item_orcamento_produto_id = new TCriteria();
        ...
        $estado_orcamento_id = new TDBCombo('estado_orcamento_id', 'mini_erp', 'EstadoOrcamento', 'id', '{nome}','nome asc' , $criteria_estado_orcamento_id );
        $obs = new TText('obs');
        $orcamento_item_orcamento_produto_id = new TDBCombo('orcamento_item_orcamento_produto_id', 'mini_erp', 'Produto', 'id', '{nome}','nome asc' , $criteria_orcamento_item_orcamento_produto_id );
        ...
        $column_calculated_3 = $this->orcamento_item_orcamento_list->addQuickColumn("Valor total", '=( ( {orcamento_item_orcamento_valor} - {orcamento_item_orcamento_desconto} ) * {orcamento_item_orcamento_quantidade}  )', 'left');

        $column_calculated_3->setTotalFunction( function($values) { 
        ...
        $column_orcamento_item_orcamento_valor_transformed->setTransformer(function($value, $object, $row, $cell = null, $last_row = null)
        {
            if(!$value)
            {
                $value = 0;
        ...
        $column_orcamento_item_orcamento_desconto_transformed->setTransformer(function($value, $object, $row, $cell = null, $last_row = null)
        {
            if(!$value)
            {
                $value = 0;
        ...
        { 
            ...
            foreach ($items as $key => $item) 
            {
                ...
                if(isset($item['orcamento_item_orcamento_produto_id']) && $item['orcamento_item_orcamento_produto_id'])
                {
                    ...
                    if($produto)
                    {
                        $rowItem->orcamento_item_orcamento_produto_id = $produto->render('{nome}');
                    }
                    TTransaction::close();
                }
