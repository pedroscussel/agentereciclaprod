...
class OrcamentoFormView extends TPage
{
    ...
    public function __construct( $param )
    {
        ...
        $text1 = new TTextDisplay($orcamento->id, '', '12px', '');
        $label22 = new TLabel("Data do orçamento:", '', '12px', '');
        $text5 = new TTextDisplay(TDateTime::convertToMask($orcamento->dt_orcamento, 'yyyy-mm-dd hh:ii', 'dd/mm/yyyy hh:ii'), '', '12px', '');
        $label2 = new TLabel("Cliente:", '', '12px', '');
        $text2 = new TTextDisplay($orcamento->cliente->nome, '', '12px', '');
        $label44 = new TLabel("Vendedor:", '', '12px', '');
        $text3 = new TTextDisplay($orcamento->vendedor->nome, '', '12px', '');
        $label4 = new TLabel("Estado do orçamento:", '', '12px', '');
        $text4 = new TTextDisplay($orcamento->estado_orcamento->nome, '', '12px', '');
        $label66 = new TLabel("Valor do frete:", '', '12px', '');
        $text7 = new TTextDisplay(number_format((double)$orcamento->frete, '2', ',', '.'), '', '12px', '');
        $label6 = new TLabel("Observação:", '', '12px', '');
        $text6 = new TTextDisplay($orcamento->obs, '', '12px', '');
        ...
        $column_calculated_5 = $this->orcamento_item_orcamento_id_list->addQuickColumn("Total", '=( ({valor} - {desconto} )* {quantidade}  )', 'left');

        $column_calculated_5->setTotalFunction( function($values) { 
