...
class VendaBatchDocument extends TPage
{
    ...
    private static $formName = 'formDocument_Venda';
    ...
    public function __construct()
    {
        ...
        $obs = new TDBCombo('obs', 'mini_erp', 'TipoProduto', 'id', '{nome}','nome asc' , $criteria_obs );
        $produto_id = new TDBCombo('produto_id', 'mini_erp', 'Produto', 'id', '{nome}','nome asc' , $criteria_produto_id );
    ...
    public function onGenerate($param)
    {
        try 
        {
            ...
            if (isset($data->obs) AND ( (is_scalar($data->obs) AND $data->obs !== '') OR (is_array($data->obs) AND (!empty($data->obs)) )) ) 
            {
                ...
                $criteria->add(new TFilter('id', 'in', "(SELECT venda_id FROM venda_item WHERE produto_id in  (SELECT id FROM produto WHERE tipo_produto_id = '{$data->obs}') )"));
            }
            if (isset($data->produto_id) AND ( (is_scalar($data->produto_id) AND $data->produto_id !== '') OR (is_array($data->produto_id) AND (!empty($data->produto_id)) )) ) 
            {
                ...
                $criteria->add(new TFilter('id', 'in', "(SELECT venda_id FROM venda_item WHERE produto_id in  (SELECT id FROM produto WHERE id = '{$data->produto_id}') )"));
            }

            $filterVar = TSession::getValue("userunitid");
            $criteria->add(new TFilter('system_unit_id', '=', $filterVar));

            $objects = Venda::getObjects($criteria, FALSE);
            if ($objects)
            {
                $output = '';
                ...
                foreach ($objects as $object)
                {
                    ...
                    $objectsVendaItem_venda_id = VendaItem::where('venda_id', '=', $object->id)->load();
