...
class VendaItemTipoProdutoChart extends TPage
{
    ...
    public function __construct($param = null)
    {
        ...
        $produto_tipo_produto_id = new TDBCombo('produto_tipo_produto_id', 'mini_erp', 'TipoProduto', 'id', '{nome}','nome asc' , $criteria_produto_tipo_produto_id );

        $produto_tipo_produto_id->setSize('100%');

        $row1 = $this->form->addFields([new TLabel("Tipo de produto:", null, '14px', null)],[$produto_tipo_produto_id],[],[]);
    ...
    public function onSearch()
    {
        ...
        if (isset($data->produto_tipo_produto_id) AND ( (is_scalar($data->produto_tipo_produto_id) AND $data->produto_tipo_produto_id !== '') OR (is_array($data->produto_tipo_produto_id) AND (!empty($data->produto_tipo_produto_id)) )) )
        {
            ...
            $filters[] = new TFilter('produto_id', 'in', "(SELECT id FROM produto WHERE tipo_produto_id = '{$data->produto_tipo_produto_id}')");// create the filter 
        }
