...
class NegociacaoAdminChart extends TPage
{
    private $form; // form
    private $loaded;
    ...
    public function __construct($param = null)
    {
        ...
        $criteria_origem_negociacao_id = new TCriteria();
        $criteria_estado_negociacao_id = new TCriteria();
        ...
        $origem_negociacao_id = new TDBCombo('origem_negociacao_id', 'mini_erp', 'OrigemNegociacao', 'id', '{nome}','nome asc' , $criteria_origem_negociacao_id );
        $estado_negociacao_id = new TDBCombo('estado_negociacao_id', 'mini_erp', 'EstadoNegociacao', 'id', '{nome}','nome asc' , $criteria_estado_negociacao_id );
        $dt_inicio_negociacao = new TDate('dt_inicio_negociacao');
        $dt_fim_negociacao = new TDate('dt_fim_negociacao');
        $id = new TDBCombo('id', 'mini_erp', 'TipoNegociacao', 'id', '{nome}','nome asc' , $criteria_id );
        ...
        $system_unit_id->setSize('100%', 90);
        $origem_negociacao_id->setSize('100%');
        $estado_negociacao_id->setSize('100%');

        $row1 = $this->form->addFields([new TLabel("Cliente:", null, '14px', null)],[$cliente_id],[new TLabel("Vendedor:", null, '14px', null)],[$vendedor_id]);
        $row2 = $this->form->addFields([new TLabel("Origem da negociação:", null, '14px', null)],[$origem_negociacao_id],[new TLabel("Fase da negociação:", null, '14px', null)],[$estado_negociacao_id]);
        $row3 = $this->form->addFields([new TLabel("Data inicial:", null, '14px', null)],[$dt_inicio_negociacao],[new TLabel("Data final:", null, '14px', null)],[$dt_fim_negociacao]);
        $row4 = $this->form->addFields([new TLabel("Tipo de negociação:", null, '14px', null)],[$id],[new TLabel("Empresa:", null, '14px', null)],[$system_unit_id]);
