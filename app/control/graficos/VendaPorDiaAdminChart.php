...
class VendaPorDiaAdminChart extends TPage
{
    private $form; // form
    private $loaded;
    ...
    private static $formName = 'formChart_Venda';
    ...
    public function __construct($param = null)
    {
        ...
        $cliente_id = new TDBCombo('cliente_id', 'mini_erp', 'Pessoa', 'id', '{nome}','nome asc' , $criteria_cliente_id );
    ...
    public static function onChangetipo_produto_id($param)
    {
        try
        {
            ...
            { 
                $criteria = TCriteria::create(['tipo_produto_id' => $param['tipo_produto_id']]);
                TDBCombo::reloadFromModel(self::$formName, 'produto_id', 'mini_erp', 'Produto', 'id', '{nome}', 'nome asc', $criteria, TRUE); 
            } 
            else 
            { 
    ...
    public function onSearch()
    {
        ...
        if (isset($data->tipo_produto_id) AND ( (is_scalar($data->tipo_produto_id) AND $data->tipo_produto_id !== '') OR (is_array($data->tipo_produto_id) AND (!empty($data->tipo_produto_id)) )) )
        {
            ...
            $filters[] = new TFilter('id', 'in', "(SELECT venda_id FROM venda_item WHERE produto_id in  (SELECT id FROM produto WHERE tipo_produto_id = '{$data->tipo_produto_id}') )");// create the filter 
        }
        if (isset($data->produto_id) AND ( (is_scalar($data->produto_id) AND $data->produto_id !== '') OR (is_array($data->produto_id) AND (!empty($data->produto_id)) )) )
        {
            ...
            $filters[] = new TFilter('id', 'in', "(SELECT venda_id FROM venda_item WHERE produto_id in  (SELECT id FROM produto WHERE id = '{$data->produto_id}') )");// create the filter 
        }
